...
class ResetPasswordController extends AbstractController
{
    ...
    public function reset(Request $request, UserPasswordHasherInterface $userPasswordHasher, TranslatorInterface $translator, string $token = null): Response
    {
        ...
        if (null === $token) {
            throw $this->createNotFoundException('Aucun jeton de réinitialisation de mot de passe n\'a été trouvé.');
        }
    ...
    private function processSendingPasswordResetEmail(string $emailFormData, MailerInterface $mailer, UserRepository $userRepository): RedirectResponse
    {
        ...
        $email = (new TemplatedEmail())
            ->from(new Address('okko.network@gmail.com', 'MinimalShop'))
            ->to($user->getEmail())
            ->subject('Réinitialisation du mot de passe')
            ->htmlTemplate('email/password/reset_password.html.twig')
            ->context([
                'resetToken' => $resetToken,
            ])
        ;
